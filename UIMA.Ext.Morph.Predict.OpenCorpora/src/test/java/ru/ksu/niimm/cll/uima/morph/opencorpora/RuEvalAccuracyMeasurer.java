...
/**
 * Created with IntelliJ IDEA. User: fsqcds Date: 2/20/13 Time: 10:59 PM To
...
public class RuEvalAccuracyMeasurer {
	...
	private static boolean isCorrect(Word word) {
		...
		for (int i = 0; i < word.getWordforms().size(); ++i) {
			...
			Set<String> grammems = FSUtils.toSet(casWf.getGrammems());
	...
	private static Set<String> opCorporaGramToRuEval(String ocPOS, Set<String> ocGrammems) {
		...
		if (ocGrammems.contains("nomn") || ocGrammems.contains("voct")) {
			result.add("nom");
		} else if (ocGrammems.contains("gent") || ocGrammems.contains("gen1")
				|| ocGrammems.contains("gen2")) {
			result.add("gen");
			...
			result.add("ins");
		} else if (ocGrammems.contains("loct") || ocGrammems.contains("loc1")
				|| ocGrammems.contains("loc2")) {
			result.add("loc");
		}
