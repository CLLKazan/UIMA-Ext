...
class VPRecognizer extends JCasAnnotator_ImplBase with Logging {
  ...
  private def processSpan(jCas: JCas, spanAnno: Annotation) {
    ...
    for (vwCoord <- verbalWfs.reverse; if !attached.contains(vwCoord)) {
      ...
      if (chunkWfs != null && !chunkWfs.isEmpty) {
        attached ++= chunkWfs
        createChunkAnnotation(jCas, for ((wIndex, wf) <- chunkWfs) yield words(wIndex))
      }
    }
  }
  ...
  private def createChunkAnnotation(jCas: JCas, chunkWords: Iterable[Word]) {
    ...
    val head = iter.next()
    val depsFsArray = new FSArray(jCas, chunkWords.size - 1)
    var depsI = 0
    while (iter.hasNext) {
